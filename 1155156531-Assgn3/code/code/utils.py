...
def show_correspondence(imgA, imgB, X1, Y1, X2, Y2, file_name='result'):
    ...
    for i in xrange(X1.shape[0]):
        ...
        newImg = cv2.circle(newImg, (X1[i], Y1[i]), 9, cur_color, -1)
        newImg = cv2.circle(newImg, (X1[i], Y1[i]), 9, [0, 0, 0], 2)
        newImg = cv2.circle(newImg, (X2[i]+shiftX, Y2[i]), 9, cur_color, -1)
        newImg = cv2.circle(newImg, (X2[i]+shiftX, Y2[i]), 9, [0, 0, 0], 2)
...
def show_correspondence2(imgA, imgB, X1, Y1, X2, Y2, file_name='result'):
    ...
    for i in xrange(X1.shape[0]):
        cur_color = np.random.rand(3)
        newImg = cv2.circle(newImg, (X1[i], Y1[i]), 5, cur_color, -1)
        newImg = cv2.circle(newImg, (X2[i]+shiftX, Y2[i]), 5, cur_color, -1)
        newImg = cv2.line(newImg, (X1[i], Y1[i]), (X2[i]+shiftX, Y2[i]), cur_color, 2)
...
def evaluate_correspondence(imgA, imgB, ground_truth_correspondence_file, scale_factor, x1_est, y1_est, x2_est, y2_est, file_name='result'):
    ...
    for i in xrange(x1_est.shape[0]):
        ...
        newImg = cv2.circle(newImg, (x1_est[i]*scale_factor, y1_est[i]*scale_factor), 9, cur_color, -1)
        newImg = cv2.circle(newImg, (x1_est[i]*scale_factor, y1_est[i]*scale_factor), 9, edgeColor, 2)
        newImg = cv2.circle(newImg, (x2_est[i]*scale_factor+shiftX, y2_est[i]*scale_factor), 9, cur_color, -1)
        newImg = cv2.circle(newImg, (x2_est[i]*scale_factor+shiftX, y2_est[i]*scale_factor), 9, edgeColor, 2)
